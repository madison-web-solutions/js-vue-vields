        ...
        <FieldGroup v-model="vals" v-model:errors="errors" :config="config">
          <div class="row g-3 mb-4">
              ...
              <template v-slot="{ choice }">
                <div
                  :style="{
                    ...
                    width: '10px',
                    height: '10px',
                    'background-color': '#' + (choice as Color).hex,
                  }"
                  class="me-2"
                ></div>
                {{ choice.label }}
            ...
            <SearchField
              ...
              directory="cats"
              class="col-6"
              tooltip="This field has a custom tooltip that you can use"
            >
              <template #suggestion="{ suggestion }">
                {{ (suggestion as Cat).label }}
                <span v-if="(suggestion as Cat).age"
                  >({{ (suggestion as Cat).age }})</span
                >
              </template>
            </SearchField>
            <SelectField
            ...
            <RadioField
              ...
              name="cat"
              directory="cats"
              :inline="!!vals.inline_checks"
              :class="vals.inline_checks ? 'col-12' : 'col-6'"
            />
            <CheckboxesField
              label="Select Cats (Arr)"
              name="cats_arr"
              directory="cats"
              valueIs="array"
              :inline="!!vals.inline_checks"
      ...
      <div class="col-4">
        <p>
          <button type="button" class="btn btn-primary" @click="toggleEditMode">
            {{ editMode == "edit" ? "Stop Editing" : "Edit" }}
          </button>
          <button type="button" class="btn btn-link" @click="focusAField">
            Test Field Focusing
          </button>
        </p>
        <pre>{{ vals }}</pre>
...
const vals = ref({
  ...
  salary: 4200000,
  description:
    "<h3>Hobbies</h3><ul><li>Trampolining</li><li>Knitting</li></ul>",
  page_sections: [
    {
      content_type: "banner",
