      ...
      <div v-pclass="'tokens-list'">
        <div v-for="token in tokens" v-pclass="'token'">
          <div v-pclass="'token-content'">
            ...
            <slot
              name="pendingTokenContent"
              v-if="isPending(token)"
              :token="token"
              >{{ token.key }}</slot
            >
          </div>
...
const { editMode, modelValue, FieldWrapper, standardWrapperProps } =
  useFormField<KeysList>(coerceToKeysList, emit, propRefs, {
