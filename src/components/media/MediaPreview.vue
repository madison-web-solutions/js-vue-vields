<template>
  <div
    v-pclass="{ 'media-preview': true, 'has-thumb': backgroundImage != null }"
    :style="style"
    @click="select"
  >
    <div v-pclass="'media-preview-overlay'" v-if="showOverlay">
      <i
        v-if="iconCssClass"
        v-pclass="'media-preview-icon'"
        :class="iconCssClass"
      ></i>
      <div
        v-if="item != null && backgroundImage == null"
        v-pclass="'media-preview-title'"
      >
        {{ item.title }}
      </div>
      <div v-if="status != 'available'" v-pclass="'media-preview-status'">
        {{ status }}
      </div>
    </div>
    <button
      v-if="inspectable"
      type="button"
      v-pclass="'media-preview-inspect'"
