<template>
  <div>
    <div v-if="mode == 'library'" class="row g-3">
      ...
      <div class="col-12">
        <div
          v-pclass="'media-preview-list'"
          class="row g-2"
          @scroll="handleScroll"
        >
          <div v-for="upload in uploads" class="col-auto">
            <div v-pclass="'media-preview'" :data-upload-status="upload.status">
              <p v-if="upload.status == 'new'" v-pclass="'upload-status'">
                Queued
              </p>
              <p v-if="upload.status == 'uploading'" v-pclass="'upload-status'">
                {{ upload.progress }}%
              </p>
              <p v-if="upload.status == 'error'" v-pclass="'upload-status'">
                {{ messageBagToString(upload.errors) }}
              </p>
            </div>
          </div>
...
const searchFn = (
  page: number,
): Promise<SearchResultPage<MediaItem>> | null => {
  if (!provider) {
    return null;
  }
...
const { searchText, suggestions, canFetchMore, fetchFirstPage, fetchNextPage } =
  useSearches<MediaItem>(searchFn);
