<template>
  <div v-pclass="'media-details'">
    <div class="row g-2">
      ...
      <div
        v-pclass="'media-details-preview'"
      ...
      >
        <img
          ...
          ref="previewImage"
          :src="imageSrc"
          @click="maybeSetCropCenter"
        />
        <div v-if="iconCssClass" v-pclass="'media-details-overlay'">
          <i v-pclass="'media-details-icon'" :class="iconCssClass"></i>
        </div>
        <i
          v-if="supportCropCenter && isImage"
          v-pclass="'media-details-crop-center-left-pointer'"
          :style="cropCenterMarkerStyle.left"
          class="fas fa-caret-down"
        ></i>
        <i
          v-if="supportCropCenter && isImage"
          v-pclass="'media-details-crop-center-top-pointer'"
          :style="cropCenterMarkerStyle.top"
      ...
      <div class="col-4" data-name="details">
        <div class="mb-2">
          ...
          <p class="mb-0" v-if="imgWidth && imgHeight">
            <strong>Dimensions:</strong> {{ imgWidth }}px x {{ imgHeight }}px
          </p>
        </div>
        <FieldGroup v-model="vals" v-model:errors="errors">
          ...
          <CompoundField
            ...
            :disabled="!editable"
            class="mb-2"
            help="Hold down alt or option key and click on image to set crop center"
          >
            <div class="row g-2">
              <NumberField
                name="top"
      ...
      <div class="col-12" data-name="url">
        URL:
        <a v-if="item && item.src" :href="item.src" target="_blank">{{
          item.src
        }}</a>
        <span v-if="item && !item.src" class="text-danger"
...
const sourceVals = computed((): MediaVals => {
  return Object.assign(
    ...
    supportCropCenter.value
      ? { cropCenter: item.value?.cropCenter ?? defaultCropCenter() }
      : {},
  );
});
...
const cropCenterMarkerStyle = computed(() => {
  ...
  if (
    ...
    const cropCenterLeft =
      leftBase + (iRect.width * vals.value.cropCenter.left) / 100;
    const cropCenterTop =
      topBase + (iRect.height * vals.value.cropCenter.top) / 100;
