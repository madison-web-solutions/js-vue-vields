...
const defaultCheckQueue: {
  rawValue: Ref<unknown>;
  defaultProp: Ref<unknown>;
...
export function getCurrentValue(
  nameOrIndex?: string | number | undefined,
): Ref<FormValue> {
  const valueLens = inject(symbols.valueLens, undefined);

  return computed(
    (): FormValue | FormValue[] | Record<string, FormValue> | undefined => {
      return valueLens
        ? getValueFromLensGeneral(valueLens, nameOrIndex)
        : undefined;
    },
  );
}
...
export function getCurrentErrors(
  nameOrIndex?: string | number | undefined,
): Ref<MessageBag | MessageBag[] | Record<string, MessageBag> | undefined> {
  const errorsLens = inject(symbols.errorsLens, undefined);

  return computed(
    (): MessageBag | MessageBag[] | Record<string, MessageBag> | undefined => {
      return errorsLens
        ? getValueFromLensGeneral(errorsLens, nameOrIndex)
        : undefined;
    },
  );
}

export function useFormField<ValueType extends FormValue>(
  valueCoerceFn: (val: unknown) => ValueType,
  emit: FieldEmitType<ValueType>,
  propRefs: UseFormFieldPropRefs,
  opts?: UseFormFieldOpts,
) {
  ...
  onBeforeMount(() => {
    if (propRefs.default) {
      queueDefaultCheck(rawValue, propRefs.default, (val: unknown) =>
        setNewValue(valueCoerceFn(val)),
      );
    }
  });
  ...
  const FieldWrapper =
    inject(symbols.fieldWrapperComponent) || StandardFieldWrapper;
...
export const useExtendsEditMode = (
  editModeProp: Ref<EditMode | undefined> | undefined,
): Ref<EditMode> => {
  const injectedEditMode = inject(symbols.editMode, undefined);
...
export const useExtendsConfig = (
  overrides: Ref<Partial<Config> | undefined> | undefined,
): Ref<Partial<Config>> => {
...
export const useHasCompoundValue = (
...
export const useHasChoices = (props: UseHasChoicesPropRefs) => {
  ...
  watchEffect(() => {
    if (directory != null && directory.value != null) {
      if (provider != null && provider.value != null) {
        provider.value
          .getAll(directory.value, extraParams?.value || {})
          .then((choiceListResult) => {
        ...
        console.log(
          "Warning, directory " +
            directory.value +
            " specified in field but there is no choice list provider",
        );
        directoryChoices.value = [];
      }
    }
  ...
  const isPartialChoosable = (
    obj: any,
  ): obj is { key: string | number; label?: unknown } => {
    return (
      typeof obj == "object" &&
      obj != null &&
      "key" in obj &&
...
export const useHasChoicesSingle = (
  modelValue: Ref<string | number | undefined>,
  props: UseHasChoicesPropRefs,
) => {
...
export function useFormFieldWithChoicesMultiple(
  emit: FieldEmitType<KeysList | BooleansMap>,
  propRefs: UseFormFieldHasChoicesMultiplePropRefs,
  opts?: UseFormFieldOpts,
) {
  ...
  const {
    ...
    FieldWrapper,
    standardWrapperProps,
  } = useFormField<KeysList | BooleansMap>(coerceFn, emit, propRefs, opts);
...
export function useParsesTextField<T>(
  modelValue: Ref<T | undefined>,
  inputEle: Ref<HTMLInputElement | null>,
  opts: ParsesTextFieldOptions<T>,
) {
  ...
  const change = () => {
    ...
    const inputTextValue: string = (inputEle.value.value || "").replace(
      /\s/g,
      "",
    );
    if (inputTextValue == "") {
      ...
      const clampedValue: T = opts.clamp
        ? opts.clamp(coercedValue)
        : coercedValue;
