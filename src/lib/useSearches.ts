...
export default function useSearches<T>(searchFn: (page: number) => Promise<SearchResultPage<T>> | null) {
  ...
  const searchDebounced = () => {
    if (typeof clearTimeout == 'undefined' || typeof setTimeout == 'undefined') {
      // Just do the search immediately without debouncing
      doSearch(1);
    } else {
      fetchedPages.value = null;
      clearTimeout(timeoutId);
      timeoutId = setTimeout(() => doSearch(1), 300);
    }
  };
