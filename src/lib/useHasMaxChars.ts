...
import type { HasMaxCharsFieldProps, RefsOf } from '../types';

export default function useHasMaxChars(modelValue: Ref<string>, props: RefsOf<HasMaxCharsFieldProps>) {
  const max = computed((): number | undefined => {
    if (
      props.max &&
      typeof props.max.value == "number" &&
      props.max.value >= 0
    ) {
  ...
  watch(remainingChars, () => {
    if (max.value != null) {
      ...
      remainingCharsTimeout == null ||
        window.clearTimeout(remainingCharsTimeout);
      remainingCharsTimeout = window.setTimeout(
        () => (showRemainingChars.value = false),
        1000
      );
    }
  });
