...
export const coerceToBooleansNativeMap = (
  value: any,
...
export function getValueFromLens<T>(
  lens: NamedLens<T>,
  name: string,
): T | undefined;
 ...
 */
export function getValueFromLensGeneral<T>(
  lens: Lens<T>,
  nameOrIndex?: string | number | undefined,
): T | T[] | Record<string, T> | undefined {
  if (lens.lensType == "fixed") {
    if (nameOrIndex == null) {
      ...
      console.error(
        `FixedLens does not have subvalues - cannot access subvalue with key '${nameOrIndex}'`,
      );
      return undefined;
    }
  }
  ...
  if (lens.lensType == "indexed") {
    if (nameOrIndex == null) {
      ...
      console.error(
        `Subvalues of IndexedLens are accessed by a numerical index, cannot access subvalue with string key '${nameOrIndex}'`,
      );
      return undefined;
    } else {
