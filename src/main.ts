...
const vueFieldsMsPlugin = (
  app: App,
  appConfig: Partial<AppConfigOptions>,
): void => {
  ...
  if (appConfig.registerFieldComponents === true) {
    const _prefix: string =
      appConfig.componentPrefix == null ? "" : appConfig.componentPrefix;
  ...
  if (appConfig.passwordStrengthProvider) {
    app.provide(
      symbols.passwordStrengthProvider,
      ref(appConfig.passwordStrengthProvider),
    );
  }
...
const providePasswordStrength = (
  provider: PasswordStrengthProvider | Ref<PasswordStrengthProvider>,
) => {
  provide(
    symbols.passwordStrengthProvider,
    isRef(provider) ? provider : ref(provider),
  );
};
